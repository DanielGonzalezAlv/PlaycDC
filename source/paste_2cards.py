        ...
        for directories in dirs:
            ...
            for filename in glob.glob(os.path.join(path_np_files, '*.npy')):
                ...
                if np_loop == 0:
                    ...
                    continue
                if np_loop == 1:
                    np_file1 = np.load(filename)
                    img1 = misc.imread(path_images + np_file1[2] + ".jpg")
                    np_loop = 0
                ...
                for texture in  glob.glob(os.path.join(name_dir_textures, '*.jpg')):
                    ...
                    img_aug1 = seq_det.augment_images([img1])[0] 

                    # Use textures
                    img_aug = img_aug / img_aug.max() 
                    img_aug1 = img_aug1 / img_aug1.max()

                    texture[shift[0]:shift[0]+img_aug.shape[0],shift[1]:shift[1]+img_aug.shape[1]] = img_aug
                    texture[shift[0]+200:shift[0]+img_aug.shape[0]+200,shift[1]+300:shift[1]+img_aug.shape[1]+300] = img_aug1
                    ...
                    ## rotate the texture
                    #seq_rotation = seq_rotation()
                    #seq_det_rot = seq_rotation.to_deterministic()
                    #img_rotation = seq_det_rot.augment_images([image_aug])[0]
                    ...
                    # Save images
                    file_name_img = "../Data/Textures/Images_twocards/" + str(np_file[2]) + "-"+ name_of_texture + ".jpg"
                    scipy.misc.imsave(file_name_img, image_aug)

                    # save npfiles
                    file_name_np = "../Data/Textures/np_convex_twocards/" + str(np_file[2]) + "-"+ name_of_texture 
